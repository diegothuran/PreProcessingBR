...
for iterations in range(MAX_EXECUTIONS):
    ...
    base = readBase('../basesExtraidas/adj-adv-noum-verb/unbalanced_three_class.txt') #matriz de similaridade entre os documentos, utilizando a distância entre os cosenos
    expected = readLabels('../basesExtraidas/adj-adv-noum-verb/unbalanced_three_class_saida.txt')
    # variaveis globais
    pop_size = 10  #tamanho do enxame = número de partículas
    dimensions = len(expected)  #quantidade de dimensoes em cada particula = número de documentos a serem clusterizados
    ...
    upperBounds = 2 #maior valor que uma dimensao pode tomar
...
#    times.append(end - start)
    ...
    gs.append(metrics.silhouette_score(expected, np.array(gbest.positions)))
    print("Silhouette Coefficient: %0.3f" % metrics.silhouette_score(expected, np.array(gbest.positions)))
...
csv_writer2(gs,"gs_unbalanced_three_class_saida.csv")
media =  np.array(reduce(operator.add, np.mean(np.array(resultados), axis=2)))/MAX_EXECUTIONS
csv_writer(media,"resultado_final_unbalanced_three_class_saida.csv")
