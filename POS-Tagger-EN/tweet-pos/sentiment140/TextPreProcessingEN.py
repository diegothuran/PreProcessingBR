...
def main():
    ...
    ## read corpus
    corpus_twitter = readBase('corpus/unbalanced_three_classes.csv')
    ...
    with codecs.open("results/unbalanced_three_classes_saida.txt", "w") as foutput:
    ...
    if (enable_perform_filtering_POS):
        for tweet in corpus_after_POS:
            ...
            for j in tweet:
                if (j[1] is not None):
                    if (j[1] == 'N') or (j[1] == 'A') or (j[1] == 'R') or (j[1] == 'V') or (j[1] == 'T') :
                        tok.append(j[0])
    ...
    np.savetxt('results/unbalanced_three_classes.txt',final)
